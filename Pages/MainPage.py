...
class MainPage(BasePage):
    ...
    def open_route_options(self, status_open=True):
        if status_open:
            ...
            assert self._is_aria_expanded(self.main_page_locators.OPTIONS_BTN)==True
        else:
            self._click(self.main_page_locators.OPTIONS_BTN, timeout=20, animation=True)
    ...
    def set_route_options(self, high_way=False, pay_tool=False, ferry=False, units='auto'):
        ...
        if units=='auto':
            ...
            assert self._is_option_checked(self.main_page_locators.OPTIONS_AUTOMATIC_UNITS_CHECK_STATUS)==True
        elif units=='miles':
            self._click(self.main_page_locators.OPTIONS_AUTOMATIC_MILES)
            assert self._is_option_checked(self.main_page_locators.OPTIONS_AUTOMATIC_MILES_CHECK_STATUS)==True
        elif units=='km':
            self._click(self.main_page_locators.OPTIONS_AUTOMATIC_KM)
            assert self._is_option_checked(self.main_page_locators.OPTIONS_AUTOMATIC_KM_CHECK_STATUS)==True
